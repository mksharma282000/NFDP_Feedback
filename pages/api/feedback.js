...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      const { name, role, foodRating, arrangementRating, overallRating, comments, deviceInfo } = req.body;
      ...
      return res.status(500).json({ error: "Something went wrong on the server" }); // ✅ Ensure valid JSON on error
    }
  } else {
